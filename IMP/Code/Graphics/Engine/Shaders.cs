...
class Shaders
{
    ...
    unsafe public Shaders()
    {
        ...
        lighting.Locs[(int)ShaderLocationIndex.VectorView] = GetShaderLocation(lighting, "viewPos");
        int ambientLoc = GetShaderLocation(lighting, "ambient");
        float[] ambient = new[] { 0.1f, 0.1f, 0.1f, 1.0f };
        SetShaderValue(lighting, ambientLoc, ambient, ShaderUniformDataType.Vec4);
    }
