...
class Intro : IDrawableObject
{
    ...
    unsafe public void OnDraw()
    {
        ...
        int fontSize = 80;
        Vector2 textSize = MeasureTextEx(GetFontDefault(), "IMP", fontSize, 2);
        Vector2 center = new Vector2(GetScreenWidth() / 2, GetScreenHeight()/2);
        DrawTextEx(GetFontDefault(), "IMP", new Vector2(center.X - textSize.X/2, center.Y - textSize.Y/2-50), fontSize, 2, Color.White);
        int scale = 5;
